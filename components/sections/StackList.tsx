...
function Chip({ label, category }: ChipData) {
  const hoverColor = categoryHover[category] || "var(--text-muted)";
...
export function StackList({ groups }: { groups: StackCategory[] }) {
  return (
        ...
        >
          ...
          >
            ...
            <span style={{ marginLeft: "auto", color: "var(--border)", fontSize: 11 }}>
              {g.chips.length < 10 ? `0${g.chips.length}` : g.chips.length}
            </span>
          </div>
